...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost:5173","https://kakubird.onrender.com", "http://localhost:3000"], # フロントエンドのURL
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
...
# 3. ルーターの登録 (URLのマッピング)
# =====
# 認証関連のAPI (生成、一覧、/me など) -> https://kakubird.onrender.com/api/auth/...
app.include_router(routes_auth.router, prefix="/api/auth", tags=["Authentication"])

# SQL実行関連のAPI (/run, /run_raw など) -> https://kakubird.onrender.com/api/sql/...
app.include_router(routes_execute.router, prefix="/api/sql", tags=["SQL Execution"])

# ★ システム・ディスカバリAPI (/services など) -> https://kakubird.onrender.com/api/...
