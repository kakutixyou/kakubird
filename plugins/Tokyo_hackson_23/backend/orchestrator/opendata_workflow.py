...
class OpenDataWorkflow:
 ...
 # ------------------------------------------------------------------
    ...
    def run_score(self, theme: str):
        """23区公式統計(WARD_DEMOGRAPHICS)をもとに相対評価でスコア計算"""
        cfg = self._load_theme_cfg(theme)
        ...
        # 🎯【変更点1】全区の密度(density)を計算し、トップの数値(max_density)を探す
        densities = {}
        max_density = 0.0
        ...
        for row in records:
            ...
            densities[city_name] = density

            # 最大値を更新
            if density > max_density:
                max_density = density

        # 🎯【変更点2】トップの区(max_density)を100点として、各区のスコアを計算する
        for row in records:
            city_name = row["municipality"]
            raw_count = row["raw_count"]
            ...
            # 軸1: データ品質スコア (100点満点の絶対評価)
            quality_score = (coord_rate * 50.0) + (open_rate * 50.0)

            # 軸2: 地域充実度スコア (トップの区を100点とした相対評価)
            density = densities[city_name]
            if max_density > 0:
                richness_score = (density / max_density) * 100.0
            else:
                richness_score = 0.0
            ...
            # DBへ保存
            self.db.save_ward_score(theme, city_name, raw_count, quality_score, round(richness_score, 1), total_score)

        logger.info(f"🎉 [{theme}] 全自治体のスコア計算(相対評価)・DB保存が完了しました。")
    ...
    def run_import_jsonl(self, theme: str, file_path: str):
        """
        【本番環境用】 provenance.jsonl を読み込み、
        JSON内の 'category' を元に自動でテーマを振り分けてDBに流し込む。
        """
        import json
        import hashlib
        from pathlib import Path
        ...
        path = Path(file_path)
        if not path.exists():
            logger.error(f"❌ 指定されたJSONLファイルが見つかりません: {file_path}")
            return

        logger.info(f"[Workflow/Import] 本番用JSONLインポートを開始: {file_path}")

        # テーマごとにデータを振り分けるための辞書
        ...
        try:
            with path.open("r", encoding="utf-8") as f:
                for line in f:
                    ...
                    # 🎯 JSONLの category (例: "AED") を小文字にしてテーマ名(aed)とする
                    # もし category が無ければ、コマンド引数の theme をフォールバックとして使う
                    current_theme = (row.get("category") or theme).lower()
                    if not current_theme or current_theme == "default":
                        logger.warning(f"テーマが特定できないためスキップ: {row.get('dataset_title')}")
                        continue

                    dataset_name = row.get("dataset_title") or "名称不明"
                    resource_name = row.get("resource_name") or ""
                    full_title = f"{dataset_name} - {resource_name}" if resource_name else dataset_name
                    
                    org_name = row.get("organization") or "不明"
                    municipality = org_name
                    ...
                    format_type = (row.get("format") or "UNKNOWN").upper()
                    download_url = row.get("source_url") or row.get("dataset_url") or ""
                    if not download_url:
                        continue
                        
                    item_id = row.get("package_id")
                    if not item_id:
                        item_id = hashlib.md5(download_url.encode('utf-8')).hexdigest()
                        
                    has_coords = format_type in ("GEOJSON", "KML", "SHP", "TOPOJSON")
                    
                    items_by_theme[current_theme].append({
                        "id": item_id,
                        "url": download_url,
                        "title": full_title,
                        "municipality": municipality,
            ...
            if not items_by_theme:
                logger.warning("インポート可能なデータがJSONLから見つかりませんでした。")
                return

            # テーマごとにDBへ一括登録
            total_inserted = 0
            for t, items in items_by_theme.items():
                inserted = self.db.insert_unassessed_items(theme=t, items=items)
                logger.info(f"🎉 [{t}] テーマに {inserted}件 を新規登録しました。")
                total_inserted += inserted
                
            logger.info(f"✅ JSONLインポート完了: 合計 {total_inserted}件 登録")

        except Exception as e:
            logger.error(f"❌ JSONLのインポートに失敗しました: {e}")
    # ------------------------------------------------------------------
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # 🎯 修正箇所：choicesの最後に "import_jsonl" を追加する
    parser.add_argument("action", choices=["collect", "download", "score", "population_score", "entertainment_score", "normalize_schema", "import_csv", "import_jsonl"], help="実行するアクション")
    ...
    # 🎯 追加箇所：JSONL用の引数もここにあるか確認してください
    parser.add_argument("--jsonl-path", type=str, default=None, help="一括インポートするJSONLのファイルパス (import_jsonl時のみ)")
...
def main():
    ...
    # 🎯 ここから追加
    elif args.action == "import_jsonl":
        if not args.jsonl_path:
            logger.error("⚠️ import_jsonl を実行するには --jsonl-path でファイルパスを指定してください。")
        else:
            workflow.run_import_jsonl(theme=args.theme, file_path=args.jsonl_path)
