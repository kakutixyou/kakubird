...
# テーマ別メトリクス抽出
# ──────────────────────────────────────────────
# [パッチ] 候補カラム名は services/field_mapper.py に一本化済み。
# cfg（テーマYAMLの読み込み結果）を渡すと `metric_fields` が優先される。
def extract_metric(theme: str, meta: dict, cfg: Optional[dict] = None) -> float:
    candidates = get_metric_field_candidates(theme, cfg)
    return extract_numeric_field(meta, candidates)
...
def build_snapshot_entry(item: dict, metric_key: str, cfg: Optional[dict] = None) -> dict:
    from engines.snapshot_engine import build_snapshot_entry as _build_snapshot_entry
...
def build_theme_snapshot(db_path: str, theme: str, metric_key: str, cfg: Optional[dict] = None) -> list[dict]:
    from engines.snapshot_engine import build_theme_snapshot as _build_theme_snapshot
