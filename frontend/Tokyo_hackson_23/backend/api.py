...
def _build_ward_evidence(theme: str, ward_name: str) -> Optional[WardEvidenceSummary]:
    ...
    normalized_facilities が空（normalize_schema/metrics未実行）の場合や
    モジュールが見つからない場合は None を返し、呼び出し側はそのまま
    ward detail を返し続けられるようにする（内訳が壊れて500になるのを防ぐ）。
    """
    try:
        from engines.evidence_engine import get_ward_evidence
    except ImportError:
    ...
    # if ev.facility_count == 0:
    #     # normalized_facilities にまだデータが無い（normalize_schema未実行等）
    #     return None
    ...
    return WardEvidenceSummary(
        ...
        sample_facilities=[FacilityEvidenceItem(**asdict(f)) for f in ev.sample_facilities],
    )
