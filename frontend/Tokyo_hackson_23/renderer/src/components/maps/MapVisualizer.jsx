...
export default function MapVisualizer({ selectedWardCode }) {
  ...
  const {
    ...
    categories,
    score, // 🌟 フックからスコアを取得
    toggleCategory, // 🌟 フックからトグル関数を取得
    handleMapClick,
    poiList,
  } = useMapScoring(INITIAL_CENTER);

// MapVisualizer.jsx の修正箇所
  useEffect(() => {
    if (selectedWardCode) {
      const district = TOKYO_23_DISTRICTS.find(d => d.code === selectedWardCode);
      if (district && district.lat && district.lng) {
        handleMapClick([district.lat, district.lng], 'target');
      }
    }
    // 👇 修正: eslintの警告が出ても handleMapClick はあえて外し、selectedWardCode が変わった時だけ動かす
  ...
  const handleSearchWorkplace = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        handleMapClick([lat, lon], 'workplace');
        setSearchQuery('');
  ...
  const handleAddPin = () => {
    ...
    handleMapClick(currentMapCenter, 'target');
  };
      ...
      <MapContainer center={center} zoom={14} zoomControl={false} style={{ width: '100%', height: '100%', zIndex: 0 }}>
        ...
        {candidatePins.map((pin, i) => {
          ...
          if (workplace) {
            ...
            const pinScore = getDistanceScore(walkMinutes);
            ...
            scoreUI = (
              <div style={{ margin: '12px 0', padding: '8px', backgroundColor: '#f8fafc', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                ...
                <div style={{ fontSize: '13px', fontWeight: 'bold', color: '#0f172a' }}>
                  🚶 徒歩約 {walkMinutes} 分 <span style={{ fontSize: '10px', color: '#94a3b8' }}>({Math.round(distance)}m)</span>
                </div>
          ...
          return (
            <Marker 
              ...
              eventHandlers={{ click: () => handleMapClick([pin.lat, pin.lng], 'target') }}
            >
              <Popup>
