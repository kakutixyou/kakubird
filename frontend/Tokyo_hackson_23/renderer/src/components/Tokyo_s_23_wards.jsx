...
// Tokyo_s_23_wards.jsx の上部
export default function Area23Map({ districts, selectedCode, onSelectDistrict, onSettleChange }) {
  // 🌟追加：アニメーションループ内で最新の選択状態を見るためのRef
  ...
  // 🔍 手動ズーム（Ctrl/Cmd + ホイールで探索用）。初期値をBASE_SCALEにして常時1.15倍表示
 ...
 // Tokyo_s_23_wards.jsx の useEffect 内の render 関数を探し、書き換えます

    const render = () => {
      // 🌟変更：区が選択されているかチェック
      if (selectedCodeRef.current) {
        // 区が選択中なら、星空を描かずにキャンバスを透明（クリア）にする！
        // これで後ろの WardEffectCanvas (nakano.js) が透けて見えます
        ctx.clearRect(0, 0, width, height);
      } else {
        // 未選択時は通常の星空グラデーションを描画
  ...
  // =========================================================
  // 2. ホイールイベント：Ctrl/Cmd+ホイールの時だけズーム、それ以外は
  //    内部スクロール（収束アニメーション）に委ねる（passive:false必須）
  // =========================================================
  useEffect(() => {
    ...
    const wheelHandler = (e) => {
      if (!(e.ctrlKey || e.metaKey)) return; // 通常ホイールはスクロール進捗の更新に使う
      ...
      // 🌟 ベースズーム(1.15)を最低ラインとして、そこから更に拡大縮小できるようにする
  ...
  // =========================================================
  // 3. スクロール進捗の計測（0〜1）：outerRef自身の内部スクロールを使用
  // =========================================================
  useEffect(() => {
    const el = outerRef.current;
    if (!el) return;
  ...
  // =========================================================
  // 4. 各区の「初期の散らばり位置・回転」を決定論的に生成
  //    （district.code から疑似乱数を作るので毎回同じ結果になる）
  // =========================================================
...
const wrapperTransform = focusedDistrict
  ...
  return (
    ...
    >
        ...
        .b-flower {
          position: absolute;
          /* padding: 15px; ← 削除！（これが透明な壁の正体です） */
          display: flex;
          flex-direction: column;
          align-items: center;
          will-change: transform, left, top, opacity;
          
          /* 🌟 追加：親の四角い当たり判定を完全に無効化 */
          pointer-events: none; 
        }
        ...
        .flower-core {
          ...
          /* 🌟 追加：この「丸」とその周辺だけをクリック可能にする */
          pointer-events: auto;
          cursor: pointer;
          position: relative;
        }

        /* 🌟 追加：丸い当たり判定（ヒットボックス）を少し広げる */
        .flower-core::after {
          content: '';
          position: absolute;
...
<div style={{ position: 'relative', width: '100%', height: '100%', maxWidth: '1000px', margin: '0 auto' }}>
          {districts.map((district, index) => {
            ...
            // const targetX = district.code === 'shibuya' ? district.x - 100 : district.x;
            // const targetY = district.code === 'shibuya' ? district.y - 500 : district.y;
