...
export default function App() {
  ...
  // 👇 🌟 新規追加: マップの表示モードと、自動ジャンプのON/OFF状態
  ...
  // 👇 🌟 新規追加: 区を選択したときの処理（自動ジャンプ制御を含む）
const handleDistrictSelect = (district) => {
  setSelectedDistrict(district);
  setIsCardVisible(true); // 👈 区を選んだらカードを表示する
  ...
  const handleCategorySelect = (key) => {
    ...
    // 自動ジャンプを防ぐため、カテゴリ選択時は直接 state を更新
      ...
      <div style={{
        ...
        <div style={{ position: 'fixed', top: '20px', right: '20px', zIndex: 600, display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          <button
            ...
            style={{
              ...
              display: viewMode === '3D' ? 'block' : 'none' // 3Dマップ時のみ表示
            }}
          >
        ...
        >
          <Area23Map
            ...
            onSelectDistrict={handleDistrictSelect} // 🌟 作成したハンドラーを使用
            onSettleChange={(settled) => setIsHeaderVisible(!settled)}
          />
        </div>

{/* 🔽 3Dマップ用 UI: 上部カテゴリ（OSMモード時は非表示） 🔽 */}
        <section style={{
...
{/* 🔽 修正: viewMode === '3D' の制限を削除し、OSM画面でも表示させる 🔽 */}
        {selectedDistrict && isCardVisible && (
          <DistrictDetailCard
            ...
            // 👇 新規追加: カード側に現在のモードを伝える
            viewMode={viewMode}
            
            // 👇 修正: バツボタン
            onClose={() => {
              setIsCardVisible(false);
              setViewMode('OSM');//元々3D
              setSelectedDistrict(null);
            }}
          />
        )}
